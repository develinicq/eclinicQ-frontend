...
export default function MiddleQueue({ doctorId: propsDoctorId, dummyMode = false, onPauseQueue, isPaused, pauseDuration, pauseStartTime, onResumeQueue }) {
	...
	const { hospitalId, doctorId: staffId } = useHospitalFrontDeskAuthStore();
	...
	const handleMarkNoShow = async () => {
		// activeActionMenuToken is the token number
		// We need to find the appointment object to get the ID
		// The queueData is already derived and available
		const appointment = queueData.find(item => item.token === activeActionMenuToken);

		if (!appointment?.id) {
			console.error("Appointment ID not found for token", activeActionMenuToken);
			return;
		}
								...
								<SampleTable
									columns={activeFilter === 'Engaged' ? [
										...
										{ key: "reason", header: "Reason For Visit", icon: true, width: 200 },
										{
											...
											width: 190,
											render: (row) => {
												const isCheckedIn = checkedInTokens[row.token] || row.status === 'CHECKED_IN' || row.status === 'WAITING';
												return (
													<div className="flex items-center jusitfy-between">
														{!isCheckedIn ? (
															row.isGrace ? (
																...
																<button
																	onClick={() => addToast({ title: "Info", message: "Reschedule logic coming soon", type: "info" })}
				...
				onConfirm={async (data) => {
					...
					try {
						...
						if (res.data?.success || res.status === 200) {
							...
							pollSlotStatus();
							fetchAppointments(selectedSlotId);
							fetchSlots(); // Refresh slots as some might be terminated
						}
					} catch (error) {
						...
						{activeActionMenuToken === 'slot_dropdown' ? (
							<div className="py-1">
								{timeSlots.length > 0 ? (
									timeSlots.map((option) => (
										...
										>
											<option.Icon className={`h-5 w-5 ${selectedSlotId === option.slotId ? 'text-white' : 'text-gray-500'}`} />
											<div>
												<div className={`font-medium ${selectedSlotId === option.slotId ? 'text-white' : 'text-gray-900'}`}>
													{option.label}
												</div>
												<div className={`text-xs ${selectedSlotId === option.slotId ? 'text-blue-100' : 'text-gray-500'}`}>
													({option.time})
												</div>
											</div>
										</button>
									...
									<div className="px-4 py-3 text-sm text-gray-500">No slots found</div>
								)}
							</div>
							...
							<>
								...
								<button
									...
									className="flex items-center gap-2 px-4 py-2 text-sm text-[#ef4444] hover:bg-red-50 text-left w-full disabled:opacity-50"
								>
