...
import { action_dot, appointement, morningQueue, nightQueue, afternoonQueue, eveningQueue, queue_download, refresh, terminate, vertical } from '../../../../public/index.js';
...
const WalkInAppointmentDrawer = ({ show, onClose, doctorId, clinicId, hospitalId, onBookedRefresh }) => {
  ...
  useEffect(() => {
    let ignore = false; const load = async () => {
      if (!show) return; if (!doctorId || (!clinicId && !hospitalId)) return; setSelectedSlotId(null); setGrouped({ morning: [], afternoon: [], evening: [], night: [] }); setTimeBuckets([]); setLoadingSlots(true); setSlotsError('');
      try {
...
const Queue = () => {
  ...
  useEffect(() => {
    let ignore = false; let inFlight = false; const sync = async () => {
      if (!selectedSlotId || inFlight) return; inFlight = true; try {
        const st = await getSlotEtaStatus(selectedSlotId); if (ignore) return; const msg = st?.message || {}; const started = msg?.slotStatus === 'STARTED' || !!(st?.started || st?.inProgress || st?.active); if (typeof msg?.currentToken === 'number') { setBackendCurrentToken(msg.currentToken); }
        ...
        if (started !== sessionStarted) { if (started) { setSessionStarted(true); setQueuePaused(false); try { await loadAppointmentsForSelectedSlot(); } catch { } } else { setSessionStarted(false); setRunStartAt(null); setBaseElapsed(0); setElapsed(0); setBackendActiveDetails(null); } }
      } catch { } finally { inFlight = false; }
    }; sync(); const id = setInterval(sync, 45000); return () => { ignore = true; clearInterval(id); };
  }, [selectedSlotId, sessionStarted, loadAppointmentsForSelectedSlot, runStartAt]);
  ...
  const timeSlots = useMemo(() => {
    const arr = [];
    if (groupedSlots.morning.length) { const f = groupedSlots.morning[0], l = groupedSlots.morning[groupedSlots.morning.length - 1]; arr.push({ key: 'morning', label: 'Morning', time: buildISTRangeLabel(f.startTime || f.slotStartTime, l.endTime || l.slotEndTime), Icon: morningQueue }); }
    if (groupedSlots.afternoon.length) { const f = groupedSlots.afternoon[0], l = groupedSlots.afternoon[groupedSlots.afternoon.length - 1]; arr.push({ key: 'afternoon', label: 'Afternoon', time: buildISTRangeLabel(f.startTime || f.slotStartTime, l.endTime || l.slotEndTime), Icon: afternoonQueue }); }
    if (groupedSlots.evening.length) { const f = groupedSlots.evening[0], l = groupedSlots.evening[groupedSlots.evening.length - 1]; arr.push({ key: 'evening', label: 'Evening', time: buildISTRangeLabel(f.startTime || f.slotStartTime, l.endTime || l.slotEndTime), Icon: eveningQueue }); }
    if (groupedSlots.night.length) { const f = groupedSlots.night[0], l = groupedSlots.night[groupedSlots.night.length - 1]; arr.push({ key: 'night', label: 'Night', time: buildISTRangeLabel(f.startTime || f.slotStartTime, l.endTime || l.slotEndTime), Icon: nightQueue }); }
    return arr;
  }, [groupedSlots]);
