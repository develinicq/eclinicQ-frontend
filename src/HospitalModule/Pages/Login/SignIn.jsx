...
// import axios from "../../lib/axios"; // REMOVED for UI-only
// import useSuperAdminAuthStore from "../../store/useSuperAdminAuthStore"; // REMOVED for UI-only
...
export default function HSignIn() {
  ...
  const { login, verifyOtp } = useHospitalAuthStore();
  ...
  React.useEffect(() => {
    ...
    // Note: keeping above line commented out or active depends on store availability. 
    // Providing valid optional chaining just in case.
    useUIStore?.getState?.().setIsLoggingOut?.(false);
    ...
    try {
      ...
      if (res.success) {
        ...
        if (!roles.includes("HOSPITAL_ADMIN")) {
          ...
          addToast({
            title: "Access Denied",
            message: "You do not have the required permissions to access the hospital module.",
            type: "error",
          });
          return;
        }
    ...
    <div className="min-h-screen flex flex-1 bg-secondary-grey50 p-3 w-full">
      ...
      <div className=" flex flex-col gap-16  bg-white px-3 p-3 w-1/2">
        <div className=" flex flex-col items-start gap-1 px-3">
          ...
          <div className="bg-warning-50 px-1 w-fit py-[2px] min-w-[18px] text-warning-400 text-[12px]">For Hospital</div>
        </div>
        ...
        <div className="flex items-center justify-center">
          <div className="w-full max-w-[500px] flex flex-col gap-6">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[14px] text-secondary-grey300 leading-tight">Login in to access hospital admin.</p>
            </div>
