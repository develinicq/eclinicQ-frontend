...
import { bookWalkInAppointment, findPatientSlots, getDoctorMe, startSlotEta, endSlotEta, getSlotEtaStatus, startPatientSessionEta, endPatientSessionEta, pauseSlotEta, resumeSlotEta } from '../../../services/authService';
...
const HQueue = () => {
  ...
  const handleToggleSession = async () => {
    ...
    try {
      ...
      setStartError(e?.response?.data?.message || e.message || 'Failed to start slot');
    } finally {
  ...
  const handleNextPatient = async () => {
    ...
    if(nextIndex >= queueData.length){ return; }
    const next = queueData[nextIndex];
    try {
      if(selectedSlotId && next?.token!=null){ await startPatientSessionEta(selectedSlotId, next.token); }
      setCurrentIndex(nextIndex);
      setRunStartAt(Date.now());
      setBaseElapsed(0);
      setElapsed(0);
    } catch(e){ console.error('Failed to start next patient', e?.response?.data || e.message); }
  };

  const handlePrevPatient = () => { if(currentIndex>0){ setCurrentIndex(currentIndex-1); setRunStartAt(Date.now()); setBaseElapsed(0); setElapsed(0); } };

  const openPauseModal = () => { setShowPauseModal(true); setPauseMinutes(null); setPauseError(''); };
  const closePauseModal = () => setShowPauseModal(false);
  const confirmPause = async () => {
    if(!sessionStarted){ setPauseError('Start the session first'); return; }
    if(!selectedSlotId){ setPauseError('No slot selected'); return; }
    if(!pauseMinutes || pauseMinutes<=0){ setPauseError('Enter minutes'); return; }
    setPauseSubmitting(true); setPauseError('');
    try { await pauseSlotEta(selectedSlotId, pauseMinutes); const ends = Date.now() + (pauseMinutes*60*1000); setPauseEndsAt(ends); setQueuePaused(true); wasRunningOnPauseRef.current=true; setShowPauseModal(false); }
    catch(e){ setPauseError(e?.response?.data?.message || e.message || 'Failed to pause'); }
    finally{ setPauseSubmitting(false); }
  };

  useEffect(()=>{ if(!queuePaused || !pauseEndsAt) return; const tick=()=>{ const remain = Math.max(0, Math.floor((pauseEndsAt-Date.now())/1000)); setPauseRemaining(remain); if(remain<=0){ setQueuePaused(false); } }; tick(); const id=setInterval(tick,1000); pauseTickerRef.current=id; return ()=> clearInterval(id); },[queuePaused, pauseEndsAt]);

  useEffect(()=>{ if(!queuePaused || !pauseEndsAt) return; const delay = Math.max(0, pauseEndsAt-Date.now()); autoResumeTimerRef.current=setTimeout(async()=>{ try{ if(selectedSlotId){ await resumeSlotEta(selectedSlotId); } } catch(e){ console.error('Auto resume failed', e?.response?.data || e.message); } finally { setQueuePaused(false); } }, delay); return ()=> clearTimeout(autoResumeTimerRef.current); },[queuePaused, pauseEndsAt, selectedSlotId]);
  ...
  return (
    <div className='p-3 sm:p-4'>
      ...
      <div className='mt-4'>
        <QueueTable data={queueData} activeIndex={currentIndex} onSelectIndex={setCurrentIndex} />
      </div>

      {/* Walk-In drawer */}
      <WalkInAppointmentDrawer show={false} onClose={()=>{}} doctorId={doctorId} clinicId={clinicId} hospitalId={hospitalId} onBookedRefresh={()=> loadAppointmentsForSelectedSlot()} />
    </div>
  );
};
