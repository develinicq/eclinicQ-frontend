...
export default function Surgery({ hospital }) {
    ...
    const [surgeries, setSurgeries] = useState(() => {
        if (Array.isArray(hospital?.surgeries) && hospital.surgeries.length > 0) {
            // Map if structure is different, for now assuming compatible
            return hospital.surgeries;
        }

        // Default dummy data from HSurgeries.jsx for demo
        return [
    ...
    useEffect(() => {
        if (Array.isArray(hospital?.surgeries) && hospital.surgeries.length > 0) {
            setSurgeries(hospital.surgeries);
        }
    }, [hospital])
    ...
    // Read-only for SuperAdmin usually, but keeping delete for UI consistency with request "copy UI"
